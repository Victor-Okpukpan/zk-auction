...
void BidValidator_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void BidValidator_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_size_of_witness() {return 37;}
...
ctx->componentMemory[coffset].templateName = "BidValidator";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 1;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
BidValidator_2_create(1,0,ctx,"main",0);
BidValidator_2_run(0,ctx);
}
