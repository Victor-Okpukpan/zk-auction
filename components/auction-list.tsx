...
export default function AuctionList() {
  ...
  const handlePlaceBid = () => {
    ...
    console.log('Placing bid:', { auctionId: selectedAuction?.id, bidAmount });
    setIsOpen(false);
    setBidAmount('');
  };
            ...
            <div className="mt-4">
              ...
              <div className="mt-4">
                ...
                <input
                  type="number"
                  step="0.01"
                  min={Number(selectedAuction?.currentBid) + Number(selectedAuction?.minBidIncrement)}
                  value={bidAmount}
                  onChange={(e) => setBidAmount(e.target.value)}
                  className="w-full rounded-lg border bg-background px-4 py-2"
                  placeholder={`Min. ${Number(selectedAuction?.currentBid) + Number(selectedAuction?.minBidIncrement)}`}
                />
              </div>
              <div className="mt-6 flex gap-3">
                <button
                  onClick={handlePlaceBid}
                  disabled={!bidAmount || Number(bidAmount) <= Number(selectedAuction?.currentBid)}
